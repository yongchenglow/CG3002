...
def learn(buffer):  
    ...
    with open('my_trained_classifier.pkl', 'rb') as fid:
        ...
        print(result)'''
        X = np.array(buffer, dtype=int, copy=True) 
...
def dataFromArduino():
    ...
    return {'buffer': buffer, 'size' : sampleSize, 'cumVoltage': cumVoltage, 'cumCurrent': cumCurrent, 'cumPower': cumPower}
