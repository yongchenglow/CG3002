# -*- coding: utf-8 -*-
"""
Created on Thu Oct 26 20:45:03 2017
...
from sklearn import preprocessing, cross_validation, metrics, neighbors, tree
...
##### segmentation #####
segmented_df = ml.segment_signal(X, 50)

##### time feature #####
time_feature_list = []       
time_feature_list = ml.time_features(segmented_df, time_feature_list)

##### labels #####
y_list = []
y = y.reshape(df.shape[0], 1)
y = ml.segment_signal(y, 50)
...
X_train, X_test, y_train, y_test = cross_validation.train_test_split(time_feature_list,
                                                   y_list, test_size = 0.25, random_state = 42)
clf = SVC()
